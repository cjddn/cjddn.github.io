...
전체탭의 내용을 지우고 
`<% list.forEach(function(data){%><%=data.title%> <%}) %>`
로 바꿔주세요 
...
forEach함수로 title을 모두 가져와 테스트 해봅시다.
...
`if`문으로 title값이 있는 데이터만 가져오도록 하고 `dueDate`가 있는 데이터만 UI가 보이도록 예외처리를 했습니다.
...
...
```
3항연산자로 query값 중 isCompleted가 있는지 체크 후 있다면 query를 findData의 인수로 보내고 없다면 빈 객체를 보냅니다.
